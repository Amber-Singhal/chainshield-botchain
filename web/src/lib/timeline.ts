...
import { fetchRecentDecisions, explorerTxUrl } from "./botchain.js";
...
export async function loadTimeline(): Promise<void> {
  ...
  rows.innerHTML = r.data
    ...
    .map((d, idx) => {
      ...
      const pillHtml = anchorPillHtml(d.anchor);
      return `
  ...
  await loadOnChainFeed();
}

async function loadOnChainFeed(): Promise<void> {
  const rows = document.getElementById("onchain-feed");
  const empty = document.getElementById("onchain-feed-empty");
  if (!rows || !empty) return;
  try {
    ...
    if (decisions.length === 0) {
      rows.innerHTML = "";
      empty.style.display = "block";
      empty.textContent = "No on-chain decisions yet.";
      return;
    }
    empty.style.display = "none";
    const words = ["ALLOW", "REQUIRE_HUMAN_CONFIRMATION", "BLOCK"];
    rows.innerHTML = decisions
      .map((d) => {
        const verdict = words[d.verdict] ?? "UNKNOWN";
        const klass = verdictKlass(verdict as Decision["verdict"]);
        return `
          <article class="timeline-item">
            <div class="timeline-time">
              ${new Date(Number(d.timestamp) * 1000).toLocaleTimeString()}
              <div class="timeline-time-sub">recorder ${escapeHtml(d.recorder.slice(0, 8))}…</div>
            </div>
            <div class="timeline-content">
              <div class="timeline-headline">
                <span class="timeline-verdict ${klass}">${escapeHtml(verdictWord(verdict as Decision["verdict"]))}.</span>
                <span class="timeline-risk">risk · <strong>${d.riskScore}</strong> / 100</span>
              </div>
              <div class="timeline-row-anchor">${anchorPillHtml({ rootHash: d.intentHash, txHash: "" })}</div>
            </div>
          </article>`;
      })
      .join("");
    ...
    if (empty) {
      empty.style.display = "block";
      empty.textContent = `On-chain feed unavailable: ${(err as Error).message}`;
    }
  }
}
