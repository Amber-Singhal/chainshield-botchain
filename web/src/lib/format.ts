...
export function formatRules(r: PolicyRules): string {
  ...
  const tokenCaps = Object.keys(r.approvalCapByToken ?? {});
  if (tokenCaps.length) parts.push(`approvalCaps=[${tokenCaps.length}]`);
...
const CHAINSCAN = "https://scan.botchain.ai";

/**
 * Pill state for a decision whose BOT Chain record transaction is being
 * submitted. Pulsing accent dot + "recording" text — no link yet because
 * there's no txHash to point at. The CSS animation lives in
 * `.anchor-pill-pending` in global.css.
 */
export function anchorPendingPillHtml(): string {
  return (
    '<span class="anchor-pill anchor-pill-pending" title="Recording decision on BOT Chain">' +
    '<span class="anchor-pill-label">BOT</span>' +
    '<span class="anchor-pill-hash">recording</span>' +
    "</span>"
  );
}
...
export function anchorPillHtml(anchor: AnchorRecord | undefined): string {
  if (!anchor || !anchor.rootHash) {
    return '<span class="anchor-missing">— not recorded on-chain</span>';
  }
  const root = escapeHtml(shortHash(anchor.rootHash));
  const tooltip = escapeHtml(
    `BOT Chain audit record\nintentHash: ${anchor.rootHash}\ntxHash:     ${anchor.txHash}\n\nClick to open on scan.botchain.ai`,
  );
  const txUrl = anchor.txHash
    ? `${CHAINSCAN}/tx/${encodeURIComponent(anchor.txHash)}`
    : `${CHAINSCAN}/search?q=${encodeURIComponent(anchor.rootHash)}`;
  return `<a class="anchor-pill" href="${escapeHtml(txUrl)}" target="_blank" rel="noopener noreferrer" title="${tooltip}"><span class="anchor-pill-label">BOT</span><span class="anchor-pill-hash">${root}</span></a>`;
}
