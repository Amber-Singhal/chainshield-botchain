import {
  ...
  presetUnknownDest,
  submitEvaluateForm,
  useOnChainPolicyInEvaluate,
} from "../lib/evaluate.js";
import { closeJsonModal, copyJsonModal } from "../lib/modal.js";
import {
  refreshOnChainPolicyCard,
  saveOnChainPolicyFromForm,
} from "../lib/onchain-policy.js";
...
function bindActionButtons(): void {
  const map: Record<string, () => void | Promise<void>> = {
    ...
    "connect-wallet": () => performConnect().catch(() => {}),
    "save-onchain-policy": () => {
      const form = document.getElementById("policy-form") as HTMLFormElement | null;
      if (form) void saveOnChainPolicyFromForm(form);
    },
    "use-onchain-policy": () => useOnChainPolicyInEvaluate(),
...
async function refreshAll(): Promise<void> {
  await Promise.all([loadPolicies(), loadTimeline(), refreshOnChainPolicyCard(), updateWalletUI()]);
}
